...
app.post("/auth/signup", async (req, res) => { const { name, email, password, department } = req.body || {}; if (!name || !validateCredentials({ email, password })) return res.status(400).json({ message: "Name, valid email, and a password of at least 8 characters are required" }); if (await findUserByEmail(email)) return res.status(409).json({ message: "An account with this email already exists" }); const role = process.env.ADMIN_EMAIL && email.toLowerCase() === process.env.ADMIN_EMAIL.toLowerCase() ? "admin" : "user"; const user = await createUser({ name: name.trim(), email: email.toLowerCase().trim(), department: String(department || "General").trim(), passwordHash: await bcrypt.hash(password, 12), role, lastLoginAt: new Date(), isActive: true }); await logActivity(user, "signup", { role }); res.status(201).json({ user: publicUser(user), token: signToken(user) }); });
...
app.get("/tickets", authRequired, async (req, res) => { const tickets = isMongoConnected ? await Ticket.find(req.user.role === "admin" ? {} : { userId: req.user._id, status: "Open" }).populate("userId", "name email department role").populate("resolver", "name email").populate("remover", "name email").sort({ submittedAt: -1 }) : readFile("tickets.json").filter((ticket) => ticket.status === "Open" && (req.user.role === "admin" || ticket.userId === String(req.user._id))); res.json(tickets); });
...
connectToDatabase().then(async () => {
  ...
  if (process.env.ADMIN_EMAIL && process.env.ADMIN_PASSWORD && !(await findUserByEmail(process.env.ADMIN_EMAIL))) {
    await createUser({
