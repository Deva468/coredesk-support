...
export function clearToken() { localStorage.removeItem(TOKEN_KEY); sessionStorage.clear(); }
...
export async function login(credentials) { const data = await request("/auth/login", { method: "POST", body: JSON.stringify(credentials) }); localStorage.setItem(TOKEN_KEY, data.token); return data.user; }
